...
OUT = os.path.expanduser("~/hackathon/demo/data")
...
def build():
    ...
    # zip_risk.json — full dataset in build_risk.py schema for behind_the_scenes.py
    lat_lon = biz_full.groupby("zip").agg(avg_lat=("latitude", "mean"), avg_lon=("longitude", "mean")).reset_index()
    yel_per_zip = biz_full.groupby("zip")["yellow_inspections"].sum().reset_index(name="yellow_count") if "yellow_inspections" in biz_full.columns else None
    grn_per_zip = biz_full.groupby("zip")["green_inspections"].sum().reset_index(name="green_count") if "green_inspections" in biz_full.columns else None

    zip_risk = zip_full.merge(lat_lon, on="zip", how="left")
    zip_risk = zip_risk.rename(columns={
        "total_red": "red_count",
        "total_critical": "critical_violations",
        "crime_count": "crime_on_food_streets",
    })
    if "yellow_count" not in zip_risk.columns:
        zip_risk["yellow_count"] = 0
    if "green_count" not in zip_risk.columns:
        zip_risk["green_count"] = 0

    # Per-business rates (used by score decomposition chart in behind_the_scenes.py)
    zip_risk["crit_per_biz"]  = zip_risk["critical_violations"] / zip_risk["businesses"].clip(lower=1)
    zip_risk["red_per_biz"]   = zip_risk["red_count"] / zip_risk["businesses"].clip(lower=1)
    zip_risk["crime_per_biz"] = zip_risk["crime_on_food_streets"] / zip_risk["businesses"].clip(lower=1)

    # Attach top_violators list per ZIP
    top_v_by_zip = {}
    for z_row in top_zips_list:
        top_v_by_zip[z_row["zip"]] = [
            {"name": b["name"], "critical_violations": b["critical_violations"], "red_inspections": b["red_inspections"]}
            for b in z_row.get("worst_businesses", [])[:3]
        ]

    zip_risk_records = []
    for _, row in zip_risk.iterrows():
        d = row.to_dict()
        d["top_violators"] = top_v_by_zip.get(str(d.get("zip", "")), [])
        # Ensure numeric types are JSON-serializable
        for k, v in d.items():
            if hasattr(v, "item"):
                d[k] = v.item()
        zip_risk_records.append(d)

    with open(f"{OUT}/zip_risk.json", "w") as f:
        json.dump(zip_risk_records, f, indent=2, default=str)
